...
public class RepositorySqls {
    ...
    "limit :start, :limit";
    public static final String TOTALCOUNT = "select count(*) from display_info d inner join product p on d.product_id = p.id inner join category c on p.category_id = c.id where c.id like if(:id = 0, '%', :id)";
}
